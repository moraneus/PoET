...
class StateManager:
        ...
        for current_state in enabled_states:
            ...
            if new_state is not None:
                if not self._is_duplicate_in_batch(new_state, newly_created_states):
                    ...
                    self._handle_duplicate_state(new_state, current_state, event)
    ...
    def _handle_duplicate_state(
        self, duplicate_state: State, source_state: State, event: Event
    ) -> None:
        """Handle duplicate state creation."""
        self.logger.trace(
    ...
    def _complete_graph_edges(self, new_states_this_step: List[State]) -> None:
        ...
        # REVERT TO ORIGINAL ALGORITHM: Only states created in the same step can complete edges
        for i, new_state in enumerate(new_states_this_step):
            if new_state.enabled:
                # Only allow completion with states created AFTER this one in the same step
