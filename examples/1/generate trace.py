...
def generate_events(trace_len: int):
    ...
    # Generate the events
    while events_count["P1"] < total_events_required or events_count["P2"] < total_events_required:
        # Decide randomly which process to add an event for, ensuring balance by the end
        process_to_use = random.choice([p for p in events_count if events_count[p] < total_events_required])
